...
\begin{abstract}
    ...
    {Daily stress affects how people feel and function, and wrist-worn devices now capture signals like heartbeat patterns (HRV), skin conductance (EDA), skin temperature, and movement that reflect changes in the body when stress rises. Models trained on these signals in the lab can be very accurate during controlled tasks, but they often falter in everyday life, where routines vary and labels are noisier. In this paper, we test whether patterns learned from the Empatica E4 during induced-stress sessions (different forms of exercises) transfer to PMData’s free-living setting with daily self-reported stress, and we ask which domain adaptation method best narrows that “lab-to-life” gap. We use the PhysioNet “Wearable Device Dataset from Induced Stress and Structured Exercise Sessions” as our labeled source domain that we will train our model on and PMData (Fitbit streams plus PMSys daily wellness reports including stress) as our target. We plan to engineer a shared feature set, aggregate E4 windows into day-level embeddings to match PMData’s labels, and compare a source-only baseline with different adaptation approaches like CORAL, DANN and other feature alignment domain adaptation techniques. Our aim is a clear, practical recipe for adapting lab-trained stress models to real-world wearables with minimal new labels.}
\end{abstract}
...
With the advancement of wearable technology, the research community maintains faith that we can be closer to quantify and measure attributes of stress. In order to reflect and assess the true quality of a person’s stress compared to how they report they feel, although today’s smartwatches possess the ability to collect various forms of data, concerns arise with the accuracy of the data collected in parallel to the device user’s actual reported feelings. Factors contributing to inaccurate data may include the quality of the wearable’s sensors, the position of the device on the user’s wrist, the wearable’s connection status and algorithms incorrectly recording data diverged from the user’s motion states. Wearable detection studies have utilized multimodal physiological data signals like Heart-Rate variability, Skin Temperature, Electrodermal Activity, and Accelerometer signs recorded from both chest-worn and wristwatches alike.The sensors within these wearables record changes from every moment which imply stress, however such models are often constrained and limited to lab conditions where stress is artificially imitated and replicated and precise without regard to real-world conditions. So in practice, when sensors are applied upon ‘free-living’ data, the models (produced within the lab) aren’t able to perform when exposed to noise, unseen movement, unforeseen environmental context, and the uncertainty of self-reported labels. To confront this, researchers are experimenting with exploring the fusion of these physiological data signals, developing models that account for additional environmental context and even going as far as to combine domain-adaptation methods that apply to common features across various unseen datasets and new devices as well. Even though progress is being made, the challenge that remains is detecting real-world stress and quantifying its accuracy for each subject user, factors include heterogeneous sensors capable of varying data collection functions, lack of standardization with data collection from sensors and subjects and limited labeled field data pushing the need for robust frameworks that can generalize lab signals for daily wearable with as little surveillance, monitoring, and human intervention as possible. 
In our research effort, we’ll test whether patterns learned from the Empatica E4 during induced-stress sessions (different forms of exercises) transfer to PMData’s free-living setting with daily self-reported stress, as we explore which domain adaptation method best narrows that “lab-to-life” gap. In order to do that, we plan to engineer a shared feature set, aggregate E4 windows into day-level embeddings to match PMData’s labels, and compare a source-only baseline with different adaptation approaches like CORAL, DANN and other feature alignment domain adaptation techniques. From there, we’ll attempt to apply classification modeling techniques to identify periods of stress presence and their causes across a wide array of behaviors including but not limited to sleeping and eating customs as well. 
