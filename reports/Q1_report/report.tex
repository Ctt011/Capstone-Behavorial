% !TEX TS-program = xelatex
% !BIB TS-program = bibtex
\documentclass[12pt,letterpaper]{article}
\usepackage{style/dsc180reportstyle}
\usepackage{booktabs}
\usepackage{float}      % for [H] placement option
\usepackage{placeins}   % for \FloatBarrier
% import dsc180reportstyle.sty

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Title and Authors
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\title{Impact of Physiological Signals and Human Behaviors recorded from a wearable device contributing to outcomes of self-reported stress}

\author{Levy Sahoo \\
  {\tt slsahoo@ucsd.edu} \\\And
  Dhyay Thakrar \\
  {\tt dthakrar@ucsd.edu} \\\And
  Camille Tran \\
  {\tt ctt011@ucsd.edu} \\\And
  Selina Zhang \\
  {\tt xiz154@ucsd.edu} \\\And
  Essie Chang \\
  {\tt e3cheng@ucsd.edu} \\\And
  Tauhidur Rahman \\
  {\tt trahman@ucsd.edu} \\}

\begin{document}
\maketitle

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Abstract and Links
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\begin{abstract}
    
    {We investigate how reliably a wrist-worn wearable can quantify stress during different kinds of stress-inducing activities. We are using the PhysioNet Wearable Device Dataset from Induced Stress and Structured Exercise Sessions (Empatica E4). The E4 provides multimodal physiological signals like heart rate and heart-rate variability, electrodermal activity (EDA), skin temperature, and accelerometry. These signals are expected to change with both physical load and cognitive strain. Our primary goals are to identify which biomarkers are most strongly associated with self-reported stress during cognitive tasks, then to test whether those same markers indicate stress during physical exercises (where explicit stress labels are absent), and finally to build models that can classify both stress level and type of activity. We first focus on the cognitive-stress protocols, segmenting E4 data into windows aligned with these cognitive tasks and using self-reported stress levels to train a regression model that predicts stress level from our engineered features. From this model, we extract a set of “stress-sensitive” biomarkers and apply them to windows from structured physical activities, training classifiers to distinguish stressed vs non-stressed segments and to classify exercise type when stress is present. By comparing performance across tasks, we aim to quantify how accurately a single wearable device can infer not just how stressed someone is, but also what kind of activity is driving that stress.}
\end{abstract}

\begin{center}
Code: \url{https://github.com/Ctt011/Capstone-Behavorial.git}
\end{center}


\clearpage

\maketoc

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Main Contents
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Introduction}
With the rapid spread of consumer wearables, there is growing hope that we can move beyond occasional self-reports and continuously quantify how stressed people are in daily life. Modern wrist-worn devices can record multiple physiological signals like heart rate and heart-rate variability, electrodermal activity (EDA), skin temperature, and movement and all these signals are known to change when stress increases. However, it is still unclear how reliably these devices capture stress across different types of stressors, and whether they can tell what kind of activity is driving that stress rather than just flagging that something is happening.

Our intuition tells us that machine learning models trained on multimodal physiological data can distinguish stress from non-stress in controlled laboratory settings, where stress is deliberately induced and labels are relatively clean. But, even in these structured environments, we do not know if different stressors may produce overlapping physiological patterns, and some signals may be driven more by physical load than by psychological strain. This makes it challenging to interpret whether a change in a wearable-derived “stress score” reflects genuine cognitive stress, physical exertion, or both.

In this project, we focus on the PhysioNet Wearable Device Dataset from Induced Stress and Structured Exercise Sessions, which includes Empatica E4 recordings collected during both cognitive stress tasks and structured physical activities. Our goals are to (1) identify which physiological biomarkers are most strongly linked to self-reported stress during cognitive tasks, (2) test whether those same biomarkers indicate stress during physical exercises where explicit stress labels are absent, and (3) build models that classify both stress level and type of activity. We first train supervised models on the cognitive protocols, using self-reported stress to learn the physiological signals that denote stress. We then examine how these signatures behave during physical exercises and train classifiers to decide whether a participant is actually stressed during activity and, when stressed, what kind of exercise they are performing. By comparing performance across tasks and modalities, we aim to quantify how reliably a single wrist-worn wearable can assess stress levels and infer likely causes of stress within this controlled experimental setting.

\section{Literature Review \& Discussion of Prior Work}

\subsection{PMData}
Lifelogging data offers valuable insights into behavior and physiology, but such datasets are often inaccessible or inconsistently structured, making standardized analysis difficult. Wearable sensing also offers rich behavioral and physiological signals, but integrating diverse data streams and making sense of them in real-world settings remains a key challenge. PMData \citep{thambawita2020pmdata} attempts to address this and introduces the combination of lifelogging data with sports activity logging, creating one of the first available datasets that combines both subjective and objective parameters. Objective parameters like heart rate, sleep, calorie, consumption, movement distance, activity sessions, and weight were captured using the Fitbit Versa 2 fitness smartwatch. Subjective parameters like wellness, fatigue, mood, sleep quality, training load, exercise readiness, injuries, and food/drink intake were logged using the PM Reporter Pro smartphone application. Additionally, a Google Form questionnaire was used to get reports about food intake through photos, and weight development. The authors conducted a set of initial experiments to demonstrate PMData’s utility, modeling next-day weight change as a three-class classification problem: weight decreases, increases, or stays the same. They trained a Random Forest and Decision Tree, and compared them to a ZeroR (majority-class baseline) under 10-fold cross-validation. Both models outperformed the baseline, with the Decision Tree yielding the strongest performance, achieving a weighted average MCC of 0.450. These results show that PMData has sufficient complexity and signal quality to support various modeling. A potential weakness with the study is the accuracy of the data. The authors observed that the Fitbit Versa stepcounter is influenced by activities other than walking/running so the estimated movement distances are slightly inaccurate. Subjective reports also raise concerns about accuracy, as it is completely dependent on the truthfulness of the reporter. However, the collected Fitbit data still provides reasonable indications of activities, and subjective information still provides great value given that psychological states can greatly influence physical performance. Ultimately, the multimodal structure of the data enables a broad range of research tasks, including classification and prediction of well-being and performance.

\subsection{Smart Devices and Wearable Technologies to Detect and Monitor Mental Health Conditions and Stress}
\cite{hickey2021smart} explore how smart devices and wearable technologies are being used to detect and monitor mental health and stress. Across 21 studies they reviewed, heart rate variability (HRV) parameters, EEG, galvanic skin response (GSR), and electrodermal activity (EDA) were used for stress and anxiety detection in wearables. They found that the use of cardiac metrics (heart rate and HRV) were the predominant markers of stress detection though, with 10 out of 15 studies assessing HRV as most useful. Most wearable devices on the market currently utilize average heart rate, which can monitor stress but not as accurately as HRV. In assessing other metrics, they found that stress detection accuracy is improved when multiple physiological signals were integrated, such as using EEG in conjunction with HRV. Overall, their review highlights the strong potentials for wearables to support stress detection but also emphasizes the gaps between research-grade sensing and what is currently available to consumers. The authors argue that multiparametric devices likely provide the greatest likelihood for robust and scalable detection, thus providing guidance on the incorporation of a variety of data for research.

\subsection{WISE Dataset}
The WISE (Wearable Stress and Exercise) dataset \citep{schlink2020wise} is a multimodal physiological dataset designed to explore how wearable sensors can measure and predict human stress and recovery patterns. It includes data collected from 36 healthy volunteers who participated in stress-inducing sessions, as well as 30–31 participants who took part in aerobic and anaerobic exercise sessions. The dataset captures a wide range of physiological signals, such as heart rate, skin temperature, respiration, electrodermal activity, and accelerometer readings. These recordings were obtained under both mental and physical stress conditions to help researchers better understand how the human body responds to varying levels of stress and exertion. By studying how wearable biosignals correlate with stress responses, the WISE dataset offers a scientific foundation for our goal of developing a personalized digital twin capable of predicting stress based on daily physiological and behavioral patterns. Its controlled environment and detailed physiological recordings provide a valuable baseline for our modeling approach, allowing us to compare induced stress responses with real-world behavioral data. The original WISE study applied signal processing and statistical techniques such as time-domain analysis, correlation, and distribution visualization to examine relationships between electrodermal activity (EDA), heart rate variability (HRV), and self-reported stress. These same analytical strategies align closely with our team’s current exploratory data analysis (EDA), which includes correlation matrices, ANOVA tests, and time-series visualizations. By benchmarking our methods against those used in WISE, we can ensure consistency and scientific rigor when working with our own datasets, such as the PMData lifelogging dataset of athletes. One of the major strengths of the WISE dataset lies in its multimodality and the presence of clearly labeled stress conditions, which enhance the accuracy of model training and validation. However, its limitations include a relatively small and homogeneous sample size, as well as the short duration of lab sessions, which may not fully represent natural variations in daily stress. Despite these constraints, WISE remains an essential reference for understanding the physiological underpinnings of stress.

\subsection{BioCV \& WESAD}
Beyond WISE, our team also reviewed other publicly available datasets that complement its insights. For example, WESAD \citep{schmidt2018wesad} provides wrist- and chest-worn physiological data from 16 subjects to detect stress, neutral, and non-stress states; Sports DB \citep{soto2021sportsdb} includes multimodal cardiorespiratory data from 81 athletes across 10 sports for fatigue and performance analysis; and BioCV \citep{moore2020biocv} offers 3D motion capture data useful for studying biomechanics and movement patterns. Together, these datasets broaden our understanding of how wearable and physiological data can be used to predict human stress, fatigue, and performance under diverse contexts. By integrating insights from these studies, our team aims to develop a robust data-driven framework capable of modeling stress through both physiological and behavioral signals for future digital twin applications. The BioCV Motion Capture dataset from the University of Bath takes a biomechanical approach to human monitoring. It includes 3D motion capture data, ground reaction forces, and photogrammetric scans from 15 participants performing various movement tasks such as walking, running, and jumping. This dataset’s strength lies in its integration of kinematic and kinetic data, which allows for precise modeling of movement efficiency, posture, and fatigue. When combined with physiological signals from datasets like WISE or WESAD, BioCV offers the opportunity to study how stress manifests not only internally (in heart rate or skin conductance) but also externally through physical form and motion.  The GLOBEM dataset \citep{zhao2023globem} extends this line of inquiry into real-world behavioral contexts. It contains multi-year data from 497 users, combining smartphone and wearable sensor logs such as location, motion, and phone usage, along with self-reported well-being scores. This large-scale dataset provides a valuable bridge between laboratory studies and everyday life, enabling models that capture long-term behavioral trends related to mental health and stress. Finally, the EPHNOGRAM dataset adds another physiological layer by recording simultaneous electrocardiogram (ECG) and phonocardiogram (PCG) data from 24 healthy adults during rest, walking, running, and biking. Its dual cardiac modalities allow for detailed investigation of how electrical and mechanical heart functions interact under different physical stressors


\section{Description of Relevant Data}
\subsection{WISE Data Description}
The Wearable Induced Stress Exercise (WISE) dataset encompasses how the body reacts to different exercises from both physical and mental examinations. Key outputs noted include physiological signals recorded from the E4 wearable such as Skin Temperature (TEMP), Heart Rate (HR), BVP (Blood Volume Pulse), Accelerometer (ACC) and finally Electrodermal Activity (EDA). The device used to collect and record this data from the subject pool is called an Empatica E4, which is a discontinued device that is no longer available for recreational purchase, however the E4 is frequently used to study how the body responds to different situations and stimuli, helping researchers understand stress, emotions, and the autonomic nervous system. The subjects involved in this study underwent 3 types of exams focused on 2 types of bicycle exercises in the aerobic and anaerobic forms as well as stressed induced mental exercises. The subjects' results were exhibited over time-series plots to notice fluctuations during the period of the examination. The ultimate goal of this effort is to monitor discrepancies between self-reported stress levels and indications observed from the collected results of the subject’s physiological response signals from these 3 activities. Using such results from time-series visuals, one potential angle we can explore is to detect periods where stress is amplified and create a classification model to exhibit anomalies to identify the causes for such stress as well as predict fluctuations in physiological signals given conditional input information about the subjects and their activity. 


\paragraph{Description of Data Preparation} 

Participants were males and females aged between 18 and 30 years. The dataset includes records from 36 healthy volunteers for stress sessions, 30 for aerobic exercise, and 31 for anaerobic exercise. Many of these subjects overlapped between all types of examinations. For simplicity and consistency within the Exploratory Data Analysis phase, we reduced the sample size down to 22 subjects to discard external factors contributing to results which may force unwanted outliers such as incorrect wristband placement, watch connection loss, incompletion of the exam protocol, and subjects with annotations to take notice of. 

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.2} 
\setlength{\tabcolsep}{6pt} 
\caption{Summary of \texttt{subject-info.csv} features and their descriptions.}
\label{tab:wise-subject-info}
\begin{tabular}{p{3.5cm}p{10.5cm}}
\toprule
\textbf{Feature} & \textbf{Description / Units} \\
\midrule
Gender & Indicates subject sex (\texttt{m} for male, \texttt{f} for female). \\
Height & Height of participant measured in centimeters (cm). \\
Age & Age of participant in years. \\
Weight & Body weight in kilograms (kg). \\
Physical Activity & Whether the subject engages in regular physical activity (\texttt{Yes} or \texttt{No}). \\
Protocol Version & The exercise protocol version conducted (\texttt{V1} for original test, \texttt{V2} for revised procedure). \\
\bottomrule
\end{tabular}
\end{table}


\paragraph{Description of Examination Types}

Each of the 22 subjects underwent the following three types of examinations: Fluctuations in physiological signals from all subjects were recorded under the following 3 categories:

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.2} 
\setlength{\tabcolsep}{6pt} 
\caption{WISE physiological signals collected: AEROBIC, ANAEROBIC, STRESS sessions.}
\label{tab:wise-signals}
\begin{tabular}{p{3.5cm}p{4cm}p{6.5cm}}
\toprule
\textbf{Source File} & \textbf{Sensor Type / Units} & \textbf{Description of Data Collected} \\
\midrule
\texttt{TEMP.csv} & Temperature sensor (\textdegree C) & Skin temperature readings expressed in degrees Celsius, recorded continuously during the session. \\
\texttt{EDA.csv} & Electrodermal activity sensor (\textmu S) & Measures changes in skin conductance as an indicator of sympathetic nervous system arousal. \\
\texttt{BVP.csv} & Photoplethysmograph (PPG) & Captures blood volume pulse signals from optical sensors for pulse waveform analysis. \\
\texttt{HR.csv} & Derived from BVP (beats per minute) & Average heart rate extracted from the BVP signal. The first row denotes session start time (UTC), and the second row specifies sampling rate (Hz). \\
\texttt{ACC.csv} & 3-axis accelerometer ($[-2g, 2g]$) & Measures wrist movement across x, y, and z axes. The recorded values are scaled by a factor of 1/64g to represent acceleration. \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Exploratory Data Overview}

The Wearable Induced Stress Exercise (WISE) dataset consists of multimodal physiological signal recordings collected using the Empatica E4 device. Each of the 22 subjects participated in three experimental sessions --- \textbf{AEROBIC}, \textbf{ANAEROBIC}, and \textbf{STRESS} --- during which multiple physiological responses were continuously logged. The dataset includes approximately 22 $\times$ 3 = 66 folders corresponding to subject-session pairs, with each containing the following time-series signal files: \texttt{TEMP.csv}, \texttt{EDA.csv}, \texttt{BVP.csv}, \texttt{HR.csv}, and \texttt{ACC.csv}. 

Each signal file contains thousands of rows representing second-level or sub-second measurements, depending on the sampling rate (e.g., 4–64 Hz). In addition, a metadata file \texttt{subject-info.csv} records subject-level demographic and behavioral attributes such as gender, age, height, weight, habitual physical activity status, and protocol version (V1 or V2). 

A summary breakdown of the dataset includes:
\begin{itemize}
    \item \textbf{Number of participants:} 22 (10 female, 12 male)
    \item \textbf{Sessions per subject:} 3 (AEROBIC, ANAEROBIC, STRESS)
    \item \textbf{Sensor modalities:} 5 physiological signals per session
    \item \textbf{Sampling rates:} EDA (4 Hz), TEMP (4 Hz), BVP (64 Hz), HR (1 Hz), ACC (32 Hz)
    \item \textbf{Total number of signal files:} 330 across all subjects and conditions
\end{itemize}

\vspace{0.5em}
Overall, this yields millions of time-stamped physiological measurements suitable for continuous stress monitoring and analysis.

\subsubsection{Relevance of the Dataset to the Research Problem}
The WISE dataset is particularly well-suited for modeling and detecting stress due to its high temporal resolution, multimodal nature, and controlled experimental design. Each subject underwent both physical and cognitive stress conditions, allowing for a comprehensive examination of autonomic responses under varying stimuli. The inclusion of synchronized EDA, HR, and BVP signals enables investigation into stress-induced changes in skin conductance and cardiovascular dynamics, while accelerometer and temperature data provide contextual information about physical activity and environmental effects.

Furthermore, the data’s structure supports the training of supervised classification models to predict stress levels based on raw physiological signals, making it ideal for testing generalizable models of human stress recognition and activity context.

\subsubsection{Limitations of the Dataset}
Despite its richness, several limitations exist in the WISE dataset:
\begin{itemize}
    \item \textbf{Sample size:} The dataset contains only 22 participants, which limits generalizability across populations.
    \item \textbf{Selection bias:} Most subjects are young adults (ages 18–30) with similar fitness levels, reducing demographic diversity.
    \item \textbf{Incomplete recordings:} Certain subjects exhibit missing or truncated signals due to wristband disconnection or data loss during transmission.
    \item \textbf{Sensor noise:} EDA and ACC signals are susceptible to motion artifacts and external disturbances, especially during high-intensity sessions.
    \item \textbf{Limited labels:} Stress episodes are not precisely annotated on a continuous scale, requiring indirect inference from session type or secondary cues.
\end{itemize}

Even with these limitations, the dataset provides a robust foundation for stress detection and multimodal physiological modeling under controlled laboratory settings.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{S05_STRESS_signals.png}
    \caption{Physiological response signals for Subject S05 during the STRESS protocol. 
    Each subplot corresponds to one of the five recorded modalities (TEMP, HR, ACC, EDA, BVP). 
    Red-shaded intervals represent annotated stress-inducing periods where sympathetic activity is heightened. 
    Observable trends include an initial rise in skin temperature, sharp fluctuations in heart rate, and 
    increases in electrodermal activity concurrent with stress stimuli.}
    \label{fig:s05_stress_signals}
\end{figure}

\subsubsection{Physiological Signal Trends under Stress}
Figure~\ref{fig:s05_stress_signals} illustrates how multiple physiological modalities respond to mental stress induction.
Distinct patterns can be observed:
\begin{itemize}
    \item \textbf{Heart Rate (HR):} Rapid spikes followed by short recovery periods reflect acute autonomic arousal.
    \item \textbf{EDA:} Gradual increases in skin conductance correspond to sympathetic nervous activation.
    \item \textbf{ACC:} Low movement variance indicates minimal motion interference during the mental stress test.
    \item \textbf{TEMP \& BVP:} Show smoother upward trends suggesting vasodilation and thermoregulation responses.
\end{itemize}
Such multimodal profiles allow downstream classification models to learn discriminative stress signatures from raw sensor data.






\subsection{PMData Description}
PMData is a multimodal personal logging dataset containing daily physiological and subjective wellness markers across 16 participants over a five month period. \citep{thambawita2020pmdata}.The dataset contains data from three sources: the Fitbit Versa 2 smartwatch provides biometric data, the PMSys sports logging app captures subjective wellness data, and Google forms collects demographic, food, and weight information. The Fitbit Versa 2 tracks physiological signals like heart rate, sleep patterns, and activity levels. The PMSys app allows participants to log daily metrics including stress, fatigue, mood, and readiness. Google Forms adds context through demographic information and lifestyle tracking. 

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.2} 
\setlength{\tabcolsep}{6pt} 
\caption{Summary of PMData sources and number of entries.}
\label{tab:pmdata-summary}
\begin{tabular}{p{3.5cm}p{4cm}p{6.5cm}}
\toprule
\textbf{Source} & \textbf{Files / Reports} & \textbf{Examples of Data Collected} \\
\midrule
Fitbit Versa~2 & $\sim$21{,}000{,}000\,heart rate samples & Heart rate, sleep score, steps, calories, and time in heart rate zones. \\
PMSys App & 1{,}747\,wellness reports & Stress, fatigue, mood, readiness, sleep duration, and sleep quality. \\
Google Forms & 1{,}569\,daily entries & Weight, alcohol, water intake, and meals. \\
\bottomrule
\end{tabular}
\end{table}

Each particpant's folder contains Fitbit JSON files, PMSys CSV files, and google form data. All files include timestamps, which make it possible to align entries by date. 

\paragraph{Processing Steps.} We used the PMSys \texttt{wellness.csv} file to obtain our main target variable,the daily \texttt{stress} score (1--5 scale). 
Fitbit data, which is collected every minute, was summarized into daily averages 
and totals such as mean heart rate, total active minutes, and previous-night sleep score. We then merged Fitbit and PMSys data by participant and date to create a consistent daily dataset.

Fitbit data is collected at a high frequency, while PMSys stress ratings occur once a day. Aggregating Fitbit signals by day ensures that each stress label is paired 
with corresponding daily physiological data. This makes the datasets comparable and helps our model learn relationships between daily stress levels and overall physiological patterns.


\FloatBarrier
\begin{table}[htbp]
\centering
\renewcommand{\arraystretch}{1.2} % add vertical spacing
\setlength{\tabcolsep}{6pt}       % adjust horizontal padding
\caption{Relevant PMData columns used in the analysis.}
\label{tab:pmdata-cols}
\begin{tabular}{p{3.2cm}p{3.2cm}p{7.8cm}}
\toprule
\textbf{File} & \textbf{Column Name} & \textbf{Description / Use} \\
\midrule
\texttt{wellness.csv} & \texttt{stress} & Target variable (1--5 scale) of self-reported daily stress; used as ground truth. \\
                      & \texttt{sleep\_duration\_h} & Total hours of sleep reported each night. \\
                      & \texttt{sleep\_quality} & Subjective rating of sleep quality. \\[4pt]
\texttt{heart\_rate.json} & -- & Minute-level heart rate readings used to compute daily averages and variability. \\
\texttt{time\_in\_heart\_rate\_zones.json} & -- & Duration spent in heart-rate zones (fat burn, cardio, peak); indicator of intensity and stress response. \\
\texttt{resting\_heart\_rate.json} & -- & Daily resting heart rate; used to track baseline changes. \\
\texttt{sleep.json} & -- & Detailed sleep-phase data (light, deep, REM, awake). \\
\texttt{sleep\_score.csv} & -- & Overall sleep-quality score (0–100) summarizing duration, composition, and restfulness. \\
\bottomrule
\end{tabular}
\end{table}



\paragraph{Exploratory Data Analysis}
Basic exploration of PMData shows that:
\begin{itemize}
    \item Most stress ratings cluster around 2-3, suggesting mild to moderate stress on average.
    \item Participants with higher average heart rates also tend to report higher stress levels.
    \item Days with lower sleep quality often correspond to slightly higher stress and fatigue.
\end{itemize}

These observations support the idea that physiological patterns (heart rate and sleep) 
relate to subjective stress levels, making PMData suitable for modeling daily stress.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.5\linewidth]{Stress Distribution.png}
\caption{Distribution of self-reported stress ratings in the PMData sample.}
\label{fig:stress-dist}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{Avg_BeHparticipant.png}
\caption{Average behavioral signals across three participants in the PMData sample. Training load measured by session Rating of Perceived Exertion (sRPE) from PMSys app.}
\label{fig:participant-metrics}
\end{figure}


\paragraph{Relevance to Our Study.}
This dataset will be used to bridge controlled stress induction data collected with the Empatica~E4 wearable and free-living, personalized lifelog data from PMData. By combining these two sources, we can test how stress patterns learned in lab settings transfer to real-world environments. PMData provides continuous wearable signals (heart rate, sleep, and activity) together with daily self-reported stress, which allows us to adapt and evaluate lab-trained stress models for practical, real-world use.


\paragraph{Limitations.}
\begin{itemize}
    \item Small sample size (16 participants) and some missing days.
    \item Stress and wellness ratings are self-reported and may be subjective.
    \item Fitbit data can include small measurement errors or gaps.
\end{itemize}
Despite these limitations, PMData offers realistic, noisy data that is valuable 
for studying stress detection in everyday settings.

\section{Methods}

\subsection{Data Preprocessing and Segmentation}

We loaded physiological signals from 22 participants (10 male V1 protocol, 12 female V2 protocol) in the WISE stress dataset. Each participant's session was recorded using the Empatica E4 wearable device, yielding time-series data for five modalities: electrodermal activity (EDA), blood volume pulse (BVP), heart rate (HR), skin temperature (TEMP), and 3-axis accelerometry (ACC). Additionally, inter-beat interval (IBI) data was extracted from the BVP signal to compute heart rate variability (HRV) metrics.

\paragraph{Data Loading and Visualization.} Initial data loading and exploratory visualizations were adapted from the preprocessing code provided in \texttt{Wearable\_Dataset.ipynb}, which is included with the WISE dataset distribution. This notebook served as the foundation for loading signal files and generating time-series plots of physiological responses.

Protocol sessions were divided into discrete time segments based on annotated timestamps (tags) that mark transitions between experimental phases. Each segment was labeled as either \texttt{STRESS} or \texttt{REST} according to the task type:

\begin{itemize}
    \item \textbf{STRESS phases:} Stroop test (V1 only), TMCT, Real Opinion, Opposite Opinion, Subtract
    \item \textbf{REST phases:} Baseline, First Rest, Second Rest, Transition Rest periods
\end{itemize}

This segmentation yielded 196 total segments (98 STRESS, 98 REST) across all participants. Segment durations ranged from 30 seconds (short cognitive tasks) to 7 minutes (baseline resting period).

\subsection{Feature Engineering}

For each segment, we extracted a set of biomarkers summarizing the physiological response during that time window. Initially, 12 features were extracted across five signal modalities, as detailed in Table~\ref{tab:features}. After removing highly correlated features (HR\_std, HR\_max, EDA\_max) to reduce multicollinearity, 9 features were retained for model training.

\begin{table}[H]
\centering
\caption{Extracted physiological features for stress classification.}
\label{tab:features}
\begin{tabular}{p{3cm}p{4cm}p{7cm}}
\toprule
\textbf{Modality} & \textbf{Features} & \textbf{Description} \\
\midrule
HRV (from IBI) & SDNN, RMSSD & Standard deviation of inter-beat intervals (overall variability); root mean square of successive differences (beat-to-beat variability). Lower values indicate stress. \\[4pt]
EDA & mean, std, max & Average skin conductance, variability, and peak response. Higher values indicate sympathetic arousal. \\[4pt]
HR & mean, std, max & Average heart rate, variability, and peak. Elevated during stress. \\[4pt]
TEMP & mean, std & Average skin temperature and variability. May rise with arousal. \\[4pt]
ACC & mean, std & Movement intensity and variability (computed via moving average filter). Fidgeting increases under stress. \\
\bottomrule
\end{tabular}
\end{table}

Features were computed by sampling signals at their native sampling rates (EDA: 4 Hz, TEMP: 4 Hz, BVP: 64 Hz, HR: 1 Hz, ACC: 32 Hz) and calculating summary statistics over the entire segment duration.

\subsection{Machine Learning Pipeline}

We formulated stress detection as a binary classification problem, where the goal is to predict whether a given segment corresponds to a STRESS or REST state based on the extracted features.

\paragraph{Data Preparation.} The dataset was split into 80\% training and 20\% testing sets using stratified sampling to preserve class balance. To handle missing HRV values (which occur in very short segments where insufficient heartbeats are recorded), we applied mean imputation.

\paragraph{Preprocessing.} All features were standardized to zero mean and unit variance using \texttt{StandardScaler} to ensure that features with different scales contribute equally to the model.

\paragraph{Classification Model.} We trained a Random Forest classifier with default hyperparameters (100 trees, Gini impurity criterion). Random Forest was chosen for its ability to handle nonlinear relationships, automatically rank feature importance, and provide robust predictions on noisy wearable sensor data.

\paragraph{Evaluation Metrics.} Model performance was evaluated on the held-out test set using:
\begin{itemize}
    \item Accuracy: Overall proportion of correct predictions
    \item Precision: Proportion of predicted STRESS segments that are truly STRESS
    \item Recall: Proportion of true STRESS segments correctly identified
    \item F1-score: Harmonic mean of precision and recall
    \item Confusion matrix: Breakdown of true positives, false positives, true negatives, and false negatives
\end{itemize}

The entire preprocessing and classification pipeline was implemented using \texttt{scikit-learn} in Python.

\subsection{Statistical Analysis}

To validate which biomarkers are significantly associated with stress, we conducted paired t-tests comparing STRESS vs REST segments for each feature. We used a significance threshold of $p < 0.05$ and corrected for multiple comparisons where appropriate. This analysis complements the machine learning approach by identifying which physiological signals exhibit statistically reliable changes under stress induction.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% RESULTS SECTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Results}

\subsection{Self-Reported Stress Levels}

Participants self-reported stress levels on a scale of 0--10 after each protocol phase. Figure~\ref{fig:stress_combined} shows the mean self-reported stress across all participants for V1 and V2 protocols.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{figure/stress_combined_figure.png}
\caption{Self-reported stress patterns. (A) Mean stress levels by protocol phase for V1 and V2 protocols. TMCT consistently induced the highest stress (6.39 for V2, 5.08 for V1), followed by Subtract and Opinion tasks. (B) Distribution of stress levels comparing stressful tasks (TMCT, Stroop, Opinion, Subtract) vs non-stressful tasks (Baseline, Rest periods). Violin plots show full distributions with individual data points, means (bars), and medians (lines).}
\label{fig:stress_combined}
\end{figure}



The TMCT task (mental math with distracting noise and verbal responses) elicited the highest mean stress (V1: 5.08, V2: 6.39), followed by the Subtract task (V1: 4.53, V2: 4.56). Rest periods consistently showed lower stress levels (Baseline: 3.28--3.44, Rest periods: 2.39--3.67). These self-reported patterns validate the protocol design and confirm that cognitive tasks successfully induced stress.

\subsection{Physiological Signal Patterns}

Raw physiological signals were visualized for representative subjects to verify stress-related changes. Figure~\ref{fig:s04_signals} shows multimodal signals for Subject S04 across the full protocol timeline.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{figure/S04_signals.png}
\caption{Physiological responses for Subject S04 during the V1 stress protocol. Colored regions indicate STRESS phases (red: Stroop, orange: TMCT, yellow: Opinion tasks, purple: Subtract). Red dashed lines mark protocol transitions. Visible patterns include EDA spikes during TMCT, elevated heart rate during cognitive tasks, and increased accelerometer activity during speech tasks.}
\label{fig:s04_signals}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{figure/f08_signals.png}
\caption{Physiological responses for Subject f08 during the V2 stress protocol (no Stroop test). Colored regions indicate STRESS phases (orange: TMCT, yellow: Opinion tasks, purple: Subtract). Similar stress-related patterns are visible despite protocol differences between V1 and V2.}
\label{fig:f08_signals}
\end{figure}

Visual inspection revealed several consistent patterns:
\begin{itemize}
    \item \textbf{EDA:} Sharp increases during TMCT and Opinion tasks, reflecting sympathetic nervous system activation
    \item \textbf{Heart Rate:} Elevated and more variable during stress phases compared to rest
    \item \textbf{ACC:} Increased movement during speech tasks (Real/Opposite Opinion) due to fidgeting and gesturing
    \item \textbf{Temperature:} Gradual upward trends during prolonged stress exposure
\end{itemize}

\subsection{Feature Correlations}

A correlation analysis of the 16 extracted features revealed several strong relationships (Figure~\ref{fig:correlation_heatmap}).

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{figure/correlation_heatmap.png}
\caption{Correlation matrix of physiological features. High correlations (>0.8) were observed between HR\_mean and HR\_max (0.85), and between EDA\_mean and EDA\_max (0.92), indicating redundancy. HRV metrics (SDNN, RMSSD) showed moderate negative correlation with HR features.}
\label{fig:correlation_heatmap}
\end{figure}

Key findings:
\begin{itemize}
    \item HR\_mean and HR\_max are highly correlated (r = 0.85), as are EDA\_mean and EDA\_max (r = 0.92)
    \item HRV metrics (SDNN, RMSSD) show weak to moderate negative correlation with HR features, consistent with the physiological expectation that higher heart rate corresponds to lower variability
    \item Temperature features show weak correlation with other modalities, suggesting independence
\end{itemize}

These correlations informed feature selection, prompting removal of redundant max values (HR\_max, EDA\_max) to reduce multicollinearity.

\subsection{Statistical Comparison: STRESS vs REST}

Paired t-tests were conducted to determine which biomarkers significantly differ between STRESS and REST states (Table~\ref{tab:ttests}).

\begin{table}[H]
\centering
\caption{Paired t-test results comparing STRESS vs REST segments.}
\label{tab:ttests}
\begin{tabular}{lccl}
\toprule
\textbf{Biomarker} & \textbf{p-value} & \textbf{Sig.} & \textbf{Effect Direction} \\
\midrule
SDNN          & 0.0068 & **  & $\downarrow$ Lower in STRESS \\
RMSSD         & 0.7204 & ns  & $\uparrow$ Higher in STRESS \\
EDA\_mean     & 0.2403 & ns  & $\uparrow$ Higher in STRESS \\
EDA\_std      & 0.0037 & **  & $\downarrow$ Lower in STRESS \\
HR\_mean      & 0.0001 & *** & $\uparrow$ Higher in STRESS \\
TEMP\_mean    & 0.2622 & ns  & $\uparrow$ Higher in STRESS \\
ACC\_mean     & 0.0000 & *** & $\uparrow$ Higher in STRESS \\
ACC\_std      & 0.0474 & *   & $\downarrow$ Lower in STRESS \\
\bottomrule
\multicolumn{4}{l}{\small * $p < 0.05$, ** $p < 0.01$, *** $p < 0.001$, ns = not significant}
\end{tabular}
\end{table}

\textbf{HR\_mean} and \textbf{ACC\_mean} showed the strongest associations with stress, both achieving $p < 0.001$. Heart rate increased significantly during STRESS segments, consistent with autonomic arousal. Movement (ACC\_mean) also increased highly significantly, likely due to fidgeting and gesturing during cognitive tasks.

\textbf{SDNN} (overall HRV) decreased significantly during stress ($p = 0.007$), reflecting reduced heart rate variability under cognitive load. Interestingly, \textbf{RMSSD} (beat-to-beat HRV) did not show a significant difference ($p = 0.720$), suggesting that overall variability (SDNN) may be more sensitive to stress than short-term beat-to-beat fluctuations.

\textbf{EDA\_std} (electrodermal activity variability) decreased significantly during stress ($p = 0.004$), while EDA\_mean did not reach significance. This suggests that stress may be characterized more by reduced EDA variability than by elevated mean skin conductance. Similarly, \textbf{ACC\_std} decreased during stress ($p = 0.047$), possibly reflecting more constrained or repetitive movement patterns during cognitive tasks.

\subsection{Classification Performance}

The Random Forest classifier achieved \textbf{72\% accuracy} on the held-out test set, with balanced performance across both classes (Table~\ref{tab:confusion}).

\begin{table}[H]
\centering
\caption{Confusion matrix and classification metrics for stress detection.}
\label{tab:confusion}
\begin{tabular}{lcc}
\toprule
& \textbf{Predicted REST} & \textbf{Predicted STRESS} \\
\midrule
\textbf{Actual REST}   & 14 & 6  \\
\textbf{Actual STRESS} & 5  & 15 \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]
\centering
\caption{Classification performance metrics.}
\label{tab:metrics}
\begin{tabular}{lcccc}
\toprule
\textbf{Class} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-Score} & \textbf{Support} \\
\midrule
REST    & 0.74 & 0.70 & 0.72 & 20 \\
STRESS  & 0.71 & 0.75 & 0.73 & 20 \\
\midrule
\textbf{Overall Accuracy} & & & \textbf{0.72} & \textbf{40} \\
\bottomrule
\end{tabular}
\end{table}

The model correctly identified 15 out of 20 STRESS segments (75\% recall) and 14 out of 20 REST segments (70\% recall). The slightly higher recall for STRESS is desirable for stress monitoring applications, where missing a stress episode is more costly than a false alarm.

\subsection{Feature Importance}

Feature importance scores from the Random Forest model reveal which biomarkers contribute most to stress classification (Figure~\ref{fig:feature_importance}).

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figure/feature_importance.png}
\caption{Feature importance scores from Random Forest classifier. TEMP\_std, ACC\_mean, and ACC\_std are the top contributors to stress classification.}
\label{fig:feature_importance}
\end{figure}

The top three features are:
\begin{enumerate}
    \item \textbf{TEMP\_std} (temperature variability): Strongest predictor (27.1\% importance), captures thermal regulation patterns during stress
    \item \textbf{ACC\_mean} (movement): Second strongest (14.0\% importance), highly significant ($p < 0.001$), captures stress-induced fidgeting and gesturing
    \item \textbf{ACC\_std} (movement variability): Third strongest (11.4\% importance), significant ($p = 0.047$), reflects constrained movement patterns during stress
\end{enumerate}

Notably, while HR\_mean and ACC\_mean showed the strongest statistical significance in t-tests ($p < 0.001$), the Random Forest model prioritized temperature and movement variability as the most discriminative features. SDNN (10.4\%) and RMSSD (9.3\%) ranked fourth and fifth, indicating that HRV metrics contribute moderately to classification despite SDNN's statistical significance. HR\_mean ranked eighth (6.9\%), suggesting that while heart rate reliably increases during stress, it provides less discriminative power than variability-based features in this multimodal context.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DISCUSSION SECTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Discussion}

\subsection{Interpretation of Findings}

Our results demonstrate that wearable physiological sensors can reliably detect cognitive stress with 72\% accuracy using only a small set of biomarkers. Random Forest feature importance analysis revealed that temperature variability (TEMP\_std), movement (ACC\_mean), and movement variability (ACC\_std) were the strongest predictors. While HR\_mean and ACC\_mean showed the highest statistical significance ($p < 0.001$), the machine learning model prioritized variability-based features for classification, suggesting that stress detection benefits from capturing dynamic patterns rather than static mean values.

\paragraph{Heart Rate as a Stress Biomarker.} The highly significant increase in HR\_mean during stress ($p < 0.001$) reflects sympathetic nervous system activation during cognitive load. Elevated heart rate is a hallmark of the "fight-or-flight" response and indicates increased metabolic demand and arousal. This finding confirms heart rate as a robust and easily measurable stress indicator in wrist-worn devices.

\paragraph{Movement Patterns.} The highly significant increase in accelerometer activity during stress ($p < 0.001$) likely captures fidgeting, gesturing, and postural shifts during demanding tasks (especially TMCT and Opinion tasks). This aligns with behavioral observations that stress induces motor restlessness and non-purposeful movement.

\paragraph{HRV as a Stress Biomarker.} The significant decrease in SDNN during stress ($p = 0.007$) reflects reduced overall heart rate variability under cognitive load. Interestingly, RMSSD (beat-to-beat HRV) was not significant ($p = 0.720$), suggesting that longer-term HRV measures (SDNN) may be more sensitive to cognitive stress than short-term fluctuations. This finding is consistent with prior work showing HRV as a stress indicator \citep{schlink2020wise, hickey2021smart}.

\paragraph{Temperature Variability as the Strongest Predictor.} Surprisingly, TEMP\_std emerged as the most important feature in the Random Forest model (27.1\% importance), despite temperature metrics not showing statistical significance in univariate t-tests. This suggests that temperature variability patterns, rather than mean temperature changes, provide highly discriminative information for stress classification when combined with other physiological signals. The classifier may be detecting subtle fluctuations in thermoregulation that occur during cognitive stress but are masked by high inter-subject variability in baseline skin temperature. This finding highlights the value of multivariate machine learning approaches that can capture complex interactions beyond univariate statistical tests.

\paragraph{EDA Patterns.} While EDA\_mean did not reach statistical significance, EDA\_std (variability) decreased significantly during stress ($p = 0.004$). This suggests that stress is characterized not by elevated mean skin conductance, but rather by more stable, less variable EDA patterns. The reduction in EDA variability may reflect sustained sympathetic activation with fewer transient fluctuations. Similarly, ACC\_std also decreased during stress ($p = 0.047$), indicating that both electrodermal and movement patterns become more constrained under cognitive load. These findings highlight the importance of examining variability metrics alongside mean values when analyzing stress responses.

\subsection{Comparison with Prior Work}

Our 72\% accuracy is comparable to other laboratory-based stress detection studies using wearable sensors. For example:
\begin{itemize}
    \item \cite{schmidt2018wesad} achieved 80\% accuracy on WESAD using chest-worn sensors (which provide higher-quality ECG)
    \item \cite{thambawita2020pmdata} reported 45\% accuracy on PMData for next-day weight prediction, but their task was fundamentally different (regression vs classification)
    \item \cite{hickey2021smart} reviewed 21 studies and found HRV-based stress detection accuracy ranging from 65--85\%
\end{itemize}

Our performance falls within this range despite using only wrist-worn sensors (which are less accurate than chest-worn ECG) and a relatively small sample size (22 subjects). This suggests that multimodal wearable data can provide robust stress detection even with consumer-grade devices.

\subsection{Limitations}

Several limitations should be noted:

\paragraph{Sample Size.} With only 22 participants (all young adults aged 18--30), our findings may not generalize to broader populations. Older adults, children, and individuals with chronic stress conditions may exhibit different physiological patterns.

\paragraph{Controlled Environment.} Data was collected in a laboratory setting with scripted stress protocols. Real-world stress (work deadlines, interpersonal conflict, etc.) may produce different physiological signatures and introduce confounding factors (e.g., physical activity, caffeine intake).

\paragraph{Missing HRV Data.} Short task segments (<32 seconds) often lacked sufficient heartbeats to compute reliable HRV metrics, resulting in missing values for 22 segments. While we used mean imputation, more sophisticated methods (e.g., model-based imputation or excluding very short segments) could improve robustness.

\paragraph{Feature Selection.} We manually selected features based on domain knowledge and correlation analysis. Automated feature engineering (e.g., deep learning on raw signals) might uncover additional predictive patterns.

\paragraph{Class Imbalance.} Although our dataset was perfectly balanced (98 STRESS, 98 REST), the definition of "stress" was protocol-driven rather than physiologically verified. Some participants may not have experienced subjective stress during designated STRESS phases, introducing label noise.

\subsection{Implications for Future Work}

Our findings have several implications for Quarter 2:

\paragraph{Integration with PMData.} The WISE-trained stress model can now be applied to PMData's free-living wearable data to detect stress episodes in athletes' daily routines. By transferring learned features (RMSSD, ACC, HR) to Fitbit signals, we can identify periods of elevated stress and correlate them with performance, training load, and recovery.

\paragraph{Model Refinement.} Future iterations could:
\begin{itemize}
    \item Incorporate temporal context (e.g., stress persistence over time)
    \item Add demographic covariates (age, gender, fitness level)
    \item Test alternative classifiers (SVM, XGBoost, LSTM for time-series)
    \item Explore subject-specific calibration to account for individual baseline differences
\end{itemize}

\paragraph{Real-Time Stress Detection.} The lightweight feature set (9 biomarkers) and fast Random Forest inference make this approach suitable for real-time on-device stress monitoring. A production system could compute rolling window features from wearable streams and provide live stress alerts.

\paragraph{Multimodal Fusion.} Combining physiological signals with behavioral data (e.g., smartphone usage, sleep patterns from PMData) may improve accuracy and provide richer insights into stress triggers.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONCLUSION SECTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Conclusion}

This study investigated the feasibility of detecting cognitive stress from multimodal wearable physiological signals using the WISE dataset. We successfully built a binary classifier that achieves 72\% accuracy in distinguishing stress from rest states. Random Forest feature importance analysis revealed that temperature variability (TEMP\_std), movement (ACC\_mean), and movement variability (ACC\_std) were the strongest predictors, accounting for 27\%, 14\%, and 11\% of model importance respectively.

Statistical validation confirmed that HR\_mean and ACC\_mean both increase highly significantly during stress ($p < 0.001$), while SDNN decreases ($p = 0.007$), aligning with established psychophysiological theory. Additionally, we found that variability metrics (EDA\_std and ACC\_std) decrease during stress, suggesting more constrained physiological patterns under cognitive load. Notably, temperature variability emerged as the most discriminative feature in the multivariate model despite not showing univariate statistical significance, highlighting the value of machine learning approaches for capturing complex physiological interactions. These findings demonstrate that consumer-grade wrist-worn sensors can capture meaningful stress-related changes, even in controlled laboratory settings with diverse cognitive stressors.

Our work provides a foundation for Quarter 2, where we will apply these learned stress signatures to real-world behavioral data from the PMData athlete lifelogging dataset. By bridging laboratory-induced stress and free-living contexts, we aim to develop personalized stress monitoring tools that can inform training optimization and well-being interventions.

Future directions include integrating temporal dynamics, exploring subject-specific calibration, and combining physiological signals with behavioral markers to build a comprehensive digital twin model capable of predicting stress, recovery, and performance outcomes in naturalistic environments.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Reference / Bibliography
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\clearpage
\bibliography{reference}
\bibliographystyle{style/dsc180bibstyle}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Appendix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\clearpage
\makeappendix

\subsection{Code Repository Structure}

All analysis code is available in our GitHub repository: \url{https://github.com/Ctt011/Capstone-Behavorial.git}

\begin{verbatim}
behavior-cap/
├── 22subjects/             # WISE dataset (physiological signals)
│   ├── STRESS/             # Cognitive stress session data
│   ├── AEROBIC/            # Aerobic exercise data
│   └── ANAEROBIC/          # Anaerobic exercise data
├── WISE_Stress_EDA.ipynb   # Main analysis notebook
├── WISE statistical analysis & eda.ipynb  # Statistical tests
├── subject-info.csv        # Participant demographics
├── Stress_Level_v1.csv     # Self-reported stress (V1 protocol)
├── Stress_Level_v2.csv     # Self-reported stress (V2 protocol)
└── Q1_report/              # LaTeX report files
\end{verbatim}

\paragraph{Acknowledgment of WISE Preprocessing Code.} The WISE dataset is distributed with a reference Jupyter notebook (\texttt{WISE\_data\_files/Wearable\_Dataset.ipynb}) containing preprocessing and visualization code. We adapted portions of this code for loading physiological signals, parsing timestamp annotations, and generating exploratory time-series visualizations. The signal plots shown in Figure~\ref{fig:s05_stress_signals}, Figure~\ref{fig:s04_signals}, and Figure~\ref{fig:f08_signals} were produced using visualization methods adapted from this reference notebook. Our feature extraction and machine learning pipelines were independently developed.

\subsection{Key Code Snippets}

\subsubsection{Signal Segmentation}

The following function segments continuous physiological recordings into labeled STRESS and REST periods based on protocol timestamps:

\begin{verbatim}
def get_stress_rest_segments(subject_id, tags):
    """
    V1: Baseline(R) → Stroop(S) → Rest(R) → TMCT(S) → ...
    V2: Baseline(R) → TMCT(S) → Rest(R) → ...
    """
    segments = []

    if subject_id.startswith('S'):  # V1 protocol
        if len(tags) >= 13:
            segments.append({'start': tags[0], 'end': tags[3],
                           'label': 'REST', 'phase': 'Baseline'})
            segments.append({'start': tags[3], 'end': tags[4],
                           'label': 'STRESS', 'phase': 'Stroop'})
            # ... (additional segments)
    else:  # V2 protocol
        if len(tags) >= 10:
            segments.append({'start': tags[0], 'end': tags[2],
                           'label': 'REST', 'phase': 'Baseline'})
            # ... (additional segments)

    return segments
\end{verbatim}

\subsubsection{HRV Calculation}

Heart rate variability metrics are computed from inter-beat interval (IBI) data:

\begin{verbatim}
def calculate_hrv(ibi_data, start_time, end_time):
    """Calculate SDNN and RMSSD from IBI timestamps"""
    timestamps = ibi_data[:, 0]
    ibi_values = ibi_data[:, 1]

    # Filter segment
    mask = (timestamps >= start_time) & (timestamps <= end_time)
    segment_ibi = ibi_values[mask]

    if len(segment_ibi) < 2:
        return {'SDNN': np.nan, 'RMSSD': np.nan}

    # Overall variability
    sdnn = np.std(segment_ibi)

    # Beat-to-beat variability
    rmssd = np.sqrt(np.mean(np.diff(segment_ibi)**2))

    return {'SDNN': sdnn, 'RMSSD': rmssd}
\end{verbatim}

\subsubsection{Feature Extraction Pipeline}

Features are extracted for each segment across all modalities:

\begin{verbatim}
def extract_segment_features(subject, segment, signals, fs_dict):
    """Extract biomarkers for one time segment"""
    features = {'subject': subject, 'label': segment['label']}

    # HRV from IBI
    if 'IBI' in signals:
        hrv = calculate_hrv(signals['IBI'],
                           segment['start'], segment['end'])
        features.update(hrv)

    # EDA statistics
    if 'EDA' in signals:
        fs = fs_dict[subject]['EDA']
        idx_start = int(segment['start'] * fs)
        idx_end = int(segment['end'] * fs)
        eda = signals['EDA'][idx_start:idx_end].flatten()
        features['EDA_mean'] = np.mean(eda)
        features['EDA_std'] = np.std(eda)

    # HR, TEMP, ACC ...
    return features
\end{verbatim}

\subsection{Reproducibility Instructions}

To reproduce our analysis:

\begin{enumerate}
    \item Clone repository: \texttt{git clone https://github.com/Ctt011/Capstone-Behavorial.git}
    \item Download WISE dataset from PhysioNet and place in \texttt{22subjects/}
    \item Install dependencies: \texttt{pip install pandas numpy matplotlib seaborn scikit-learn scipy}
    \item Run \texttt{WISE\_Stress\_EDA.ipynb} in Jupyter Notebook
    \item Statistical tests notebook: \texttt{WISE statistical analysis \& eda.ipynb}
\end{enumerate}

All random seeds are fixed (\texttt{random\_state=42}) for reproducible train/test splits and model training.

\end{document}